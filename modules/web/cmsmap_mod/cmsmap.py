...
class Initialize:
    ...
    def CMSmapUpdate(self):
        ...
        if not self.ospath+".git":
            msg = "Git Repository Not Found. Please download the latest version of from GitHub repository"; report.error(msg)
            msg = "Example: git clone https://github.com/Dionach/cmsmap"; report.error(msg)
        else:
            msg = "Updating to the latest version from GitHub repository... "; report.message(msg)
            os.chdir(self.ospath)
            process = os.system("git pull")
            if process == 0 : success = True
        if success :
            msg = "is now updated to the latest version!"; report.message(msg)
        else :
            msg = " Updated could not be completed. Please download the latest version of from GitHub repository"; report.error(msg)
            msg = " Example: git clone https://github.com/Dionach/cmsmap"; report.error(msg)
    ...
    def FindCMSType(self):
        ...
        try:
            ...
            if self.force is None :                
                ...
                msg =  "Use -f to force to scan (W)ordpress, (J)oomla or (D)rupal"; report.error(msg)
                sys.exit()               
        ...
        try:
            ...
            try:
                ...
                if re.search("Installing component was successful.",htmltext):
                    msg ="Joomla Shell Plugin Installed"; report.high(msg)
                    msg = "Web Shell: "+self.url+"/components/com_joo-shell/joo-shell.php"; report.high(msg)
                    msg = "Remember to unistall Joomla Shell Component"; report.high(msg)
                    report.WriteTextFile(msg)
        ...
        try:
            ...
            try:
                ...
                if re.search("Installing drushell",htmltext):
                    msg = "Drupal Shell Module Installed"; report.high(msg)
                    msg = "Web Shell: "+self.url+"/sites/all/modules/drushell/shell.php"; report.high(msg)
                    msg = "Remember to delete Drupal Shell Module"; report.high(msg)
            except IndexError :
                msg = "Unable to install Drupal Shell Module. Check if it is already installed"
                report.verbose(msg)
        except urllib2.HTTPError, e:
            # print e.code
            pass
...
def exit(signum, frame):
    ...
    try:
        msg = "Interrupt caught. paused. Do you really want to exit?"; report.error(msg)
        if raw_input("[y/N]: ").lower().startswith('y'):
